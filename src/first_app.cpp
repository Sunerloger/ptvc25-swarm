...
namespace vk {
	...
	glm::mat4 loadMatrix(const std::string& path, const std::string& section) {
		...
		glm::mat4 M(1.0f);
		for (int r = 0; r < 4; ++r) {
			// look in whatever section the caller asked for
			std::string key = "row" + std::to_string(r);
			auto s = reader.Get(section, key, "");
			if (s.empty()) {
				std::ostringstream os;
				os << "missing key [" << section << "] " << key;
	...
	void FirstApp::loadGameObjects() {
		...
		glm::mat4 M = loadMatrix("settings.ini", "QuadTransform");
		std::cout << "QuadTransform: " << glm::to_string(M) << std::endl;
		M = loadMatrix("settings.ini", "HelmetTransform");
		std::cout << "HelmetTransform: " << glm::to_string(M) << std::endl;
