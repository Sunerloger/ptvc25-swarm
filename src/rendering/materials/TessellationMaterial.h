...
namespace vk {
	...
	class TessellationMaterial : public Material {
	   public:
		...
		float getMaxTessLevel() const {
			return maxTessLevel;
		}
		float getTessDistance() const {
			return tessDistance;
		}
		float getMinTessDistance() const {
			return minTessDistance;
		}
		float getHeightScale() const {
			return heightScale;
		}
		bool hasHeightmapTexture() const {
			return m_hasHeightmapTexture;
		}
	   ...
	   private:
		...
		uint32_t textureMipLevels = 1;	// Added for color texture
		...
		uint32_t heightmapMipLevels = 1;  // Added for heightmap texture
